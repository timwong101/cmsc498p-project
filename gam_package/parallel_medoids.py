...
class ParallelMedoids:
    ...
    def __init__(self, n_clusters=1, dist_func='euclidean', max_iter=1000, tol=0.0001,
        ...
        self.df = sqlContext.read.format('com.databricks.spark.csv').options(header='true', inferschema='true').load(
            "../mushroom-attributions-200-samples.csv")
        max_iter = 10;
        WCSoD1 = float("inf")
        approximateTrackingFlag = False
        self.medoids = self.initialSeeds(self.df, self.k)
        self.medoids = medoidsList

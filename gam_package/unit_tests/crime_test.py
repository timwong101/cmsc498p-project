...
class TestClass:
    ...
    def test_gam_bandit_crime_runtime_700_samples(self):
        local_attribution_path = '../data/crime.csv'
        g = GAM(attributions_path=local_attribution_path, n_clusters=3, cluster_method="bandit pam", num_samp=700)
        g.generate()
        print("BanditPAM 700 Runtime: ", g.duration)

    def test_gam_bandit_crime_runtime_1000_samples(self):
        local_attribution_path = '../data/crime.csv'
        g = GAM(attributions_path=local_attribution_path, n_clusters=3, cluster_method="bandit pam", num_samp=1000)
        g.generate()

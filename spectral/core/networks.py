...
from spectral.core import train
from spectral.core import costs
from spectral.core.layer import stack_layers
...
class SiameseNet:
    def __init__(self, inputs, arch, siam_reg, y_true):
        ...
        # generate layers
        self.layers = []
        from spectral.core.util import make_layer_list
    ...
    def train(self, pairs_train, dist_train, pairs_val, dist_val,
        ...
        from spectral.core.util import LearningHandler
        ...
        from spectral.core.util import train_gen
...
class SpectralNet:
    def __init__(self, inputs, arch, spec_reg, y_true, y_train_labeled_onehot,
        ...
        from spectral.core.util import get_scale
        ...
        if have_labeled:
            ...
            unlabeled_end = tf.shape(input=self.inputs['Unlabeled'])[0]
        ...
        # define loss
        self.loss = tf.compat.v1.keras.backend.sum(W * self.Dy) / (2 * batch_sizes['Unlabeled'])
        ...
        tf.compat.v1.keras.backend.get_session().run(tf.compat.v1.variables_initializer(self.net.trainable_weights))

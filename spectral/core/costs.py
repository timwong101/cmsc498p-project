...
def knn_affinity(X, n_nbrs, scale=None, scale_nbr=None, local_scale=None, verbose=False):
    ...
    if scale is None:
        ...
        if local_scale:
            ...
            if verbose:
                vals = tf.compat.v1.Print(vals, [tf.shape(input=vals), tf.shape(input=scale)], "vals, scale shape")
            vals = vals / (2*scale)
    ...
    ii = tf.range(tf.shape(input=nnInd)[0])
    ii = tf.reshape(ii, [-1, 1])
    ii = tf.tile(ii, [1, tf.shape(input=nnInd)[1]])
    ii = tf.reshape(ii, [-1, 1])
    #concatenate the indices to build the sparse matrix
    indices = tf.concat((ii,jj),axis=1)
    #assemble the sparse Weight matrix
    W = tf.SparseTensor(indices=tf.cast(indices, dtype='int64'), values=affVals, dense_shape=tf.cast(tf.shape(input=Dx), dtype='int64'))
    #fix the ordering of the indices
    W = tf.sparse.reorder(W)
    #convert to dense tensor
    W = tf.sparse.to_dense(W)
    #symmetrize
    W = (W+tf.transpose(a=W))/2.0;
